---- I have named the database as CS504 Lib. Management----
...
Material_ID NUMERIC PRIMARY KEY,
Title VARCHAR(255) NOT NULL,  
Publication_date DATE,
Catalog_ID NUMERIC REFERENCES "Catalog"(Catalog_ID), 
Genre_ID NUMERIC REFERENCES Genre(Genre_ID)  
);

CREATE TABLE Authorship(
Authorship_ID NUMERIC PRIMARY KEY,
Author_ID NUMERIC REFERENCES Author(Author_ID),
Material_id NUMERIC REFERENCES Material(Material_ID));
...
CREATE TABLE Borrow(
Borrow_ID NUMERIC PRIMARY KEY,
Borrow_Date Date,
Due_Date DATE,
Return_Date DATE,
Material_ID NUMERIC REFERENCES Material(Material_ID),
Member_ID NUMERIC REFERENCES Member(Member_ID),
Staff_ID NUMERIC REFERENCES Staff(Staff_ID));
...
(SELECT Borrow_ID
 FROM Borrow
 JOIN Material ON Borrow.Material_ID = Material.Material_ID
 WHERE Material.Title = 'Harry Potter and the Philosopher''s Stone' AND Borrow.Return_Date IS NULL
 ORDER BY Borrow.Borrow_Date DESC
 LIMIT 1);

SELECT Borrow.Borrow_ID, Material.Title, Borrow.Material_ID, Borrow.Member_ID, Borrow.Staff_ID, Borrow.Borrow_Date, Borrow.Due_Date, Borrow.Return_Date
FROM Borrow
JOIN Material ON Borrow.Material_ID = Material.Material_ID
WHERE Material.Title = 'Harry Potter and the Philosopher''s Stone' AND Borrow.Return_Date = '2023-04-01'
ORDER BY Borrow.Borrow_Date DESC
LIMIT 1;
